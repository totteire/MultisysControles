...
function _init(tabNum){
    ...
    function reloadContent(){
        ...
        if(tab.page == 'option.php'){
            ...
            // NbRes
            $("#tabOpt #nbRes").change(function(){
                window.nbRes = $("#tabOpt #nbRes").val();
                $.get("setNbRes.php", {'nbRes': window.nbRes});
            })
            $("#tabOpt #nbRes").val(window.nbRes);
    ...
    function getNbResFromDb(){
        $.get('getNbResFromDb.php', function(data) {
            window.nbRes = data.nbRes;
            $("#tabOpt #nbRes").val(data.nbRes);
        }, "json");
    }
    ...
    function refreshTabClick(){
        ...
        $('ul.ui-tabs-nav a').click(function(){
            ...
            if(tab.needReload == "true"){
                $('.ui-tabs-panel:visible').html("<h1 style='margin-left:10%;'>Chargement ...</h1>");
                console.log("allo koi!");
                // RELOAD PAGE
                $('.ui-tabs-panel:visible').load(
                    tab.page,
                    function(){
                        reloadContent();
